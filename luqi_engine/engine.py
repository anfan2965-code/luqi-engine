...
from luqi_engine.core.rng import SeededRNGManager, NarrativeSeedHierarchy
...
_REPETITION_MIN_TEXT_LENGTH: int = 8
_REPETITION_PATTERN_SPECS: list = [
    (r'(.{2,8})[：:]\s*\1[：:]\s*\1', 3),
    (r'(.{2,10})[，,]\s*\1([，,]\s*\1){1,}', 3),
    (r'(（内心[：:].*?）)\s*\1', 2),
    (r'(.{3,15})\s+\1\s+\1', 3),
]
_REPETITION_MIN_CUTOFF_POSITION: int = 6
_REPETITION_NGRAM_SIZES: list = [6, 4]
_REPETITION_GAP_MULTIPLIER: int = 3
_REPETITION_POSITION_MULTIPLIER: int = 2
_VOICE_RENDER_DEFAULT_SEED: int = 0
_NARRATIVE_DOC_ID_PREFIX: str = "narr_"
_DEFAULT_WORLD_ID: str = "default"
_SNAPSHOT_FILENAME_TEMPLATE: str = "luqi_engine_snapshot_{}.json"
_SINGLE_CHARACTER_PRIORITY_SCORE: float = 1.0
_DEFAULT_MAX_DIALOGUE_ROUNDS: int = 20
...
class LuqiEngine:
    ...
    def __init__(self, config: Optional[EngineConfig] = None,
        ...
        self._character_extractor: CharacterExtractor = CharacterExtractor(
            state_renderer=self._state_renderer,
        )
    ...
    def save_snapshot(self, path: Optional[str] = None) -> str:
        ...
        if path is None:
            ...
            path = _SNAPSHOT_FILENAME_TEMPLATE.format(timestamp)
    ...
    async def chat(
        ...
        委托给ChatOrchestrator.orchestrate()
        """
    ...
    def _truncate_repetition_loop(text: str) -> Optional[str]:
        if not text or len(text) < _REPETITION_MIN_TEXT_LENGTH:
            return None
        import re as _re
        for pattern, min_repeat in _REPETITION_PATTERN_SPECS:
            match = _re.search(pattern, text)
            if match:
                cutoff = match.start()
                if cutoff > _REPETITION_MIN_CUTOFF_POSITION:
                    return text[:cutoff]
        for ngram_size in _REPETITION_NGRAM_SIZES:
            ...
            for i in range(len(text) - ngram_size + 1):
                ...
                if ngram in seen:
                    gap = i - seen[ngram]
                    if gap <= ngram_size * _REPETITION_GAP_MULTIPLIER and i > ngram_size * _REPETITION_POSITION_MULTIPLIER:
    ...
    def _extract_personality(self, character: Any) -> Dict[str, float]:
        ...
        try:
            ...
            self._logger.warning("OCEAN人格提取失败: %s", exc)
            return {}
    ...
    def _extract_emotion_pad(self, character: Any) -> Dict[str, float]:
        ...
        try:
            ...
            self._logger.warning("PAD情感提取失败: %s", exc)
            return {}
    ...
    def _build_prompt_context(self, character: Any, mode: DialogueMode) -> Any:
        ...
        return PromptContext(
            ...
            dominant_emotion=(lambda de: de() if callable(de) else de)(character.emotion.dominant_emotion) if hasattr(character.emotion, 'dominant_emotion') else "neutral",
            memories=[],
    ...
    async def start_dialogue(
        ...
        topic: str,
        mode: DialogueMode = DialogueMode.MULTI_CHARACTER,
        max_rounds: int = _DEFAULT_MAX_DIALOGUE_ROUNDS,
    ) -> List[Dict[str, Any]]:
        """
        启动多角色对话
        """
        ...
        if mode == DialogueMode.SINGLE_CHARACTER and len(participants) == 1:
            ...
            return [{
                "round": 0,
                "speaker_id": char_id,
                "priority_score": _SINGLE_CHARACTER_PRIORITY_SCORE,
                "topic": topic,
